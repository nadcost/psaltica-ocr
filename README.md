...
The matcher also applies built-in per-app tuning for `Apostrofos`, `Isson2`, and `Oligon`; use `--icon-threshold Name=0.80`, `--score-only-nms`, or `--no-icon-size-filters` when auditing alternate thresholds.
